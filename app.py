...
def badge_html(tarifa):
    ...
    label = 'REVISAR' if tarifa == '?' else tarifa
...
def regla_dura(t, ph, hrs, pileta):
    ...
    if 'simops' in t:
        return '?', 'SIMOPS — operación simultánea → revisar'
...
def clasificar_fila(op, ph, hrs, pileta, modelo):
    ...
    # SIMOPS: si hay operación RIG activa → RIG + flag; si no → revisión + flag
    if flag_simops:
        ...
        if (tiene_mov and tiene_mat) or tiene_rig:
            return 'RIG', 'alta', 'SIMOPS con operación RIG activa', True
        return '?', 'revision', 'SIMOPS → revisar', True

    # Ambigüedad RIG+RIGS (sin SIMOPS)
    if detectar_ambiguedad(t, pileta):
        return '?', 'revision', 'Ambigüedad RIG+RIGS → revisar', False

    # Regla dura
    tarifa, motivo = regla_dura(t, ph, hrs, pileta)
    if tarifa:
        if es_pre_post and tarifa == 'RIG':
            return '?', 'revision', f'Phase {phU}: no puede ser RIG → revisar', False
        if not es_pre_post and phU and tarifa == 'DTM':
            return '?', 'revision', f'Phase {phU}: no puede ser DTM → revisar', False
    ...
    if modelo:
        ...
        if es_pre_post and tarifa == 'RIG':
            return '?', 'revision', f'Phase {phU}: no puede ser RIG (KNN) → revisar', False
        if not es_pre_post and phU and tarifa == 'DTM':
            return '?', 'revision', f'Phase {phU}: no puede ser DTM (KNN) → revisar', False
        return tarifa, conf, motivo, False

    return '?', 'revision', 'Sin modelo disponible', False

def validar_limites(df_pre, df_post, tipo_pozo):
    alertas = []
    kw_fin, kw_inicio = LIMITES_DTM.get(tipo_pozo, ([],[]))
    if not df_pre.empty:
        op = normalizar(str(df_pre.iloc[-1].get('Operation','')))
        if not any(k in op for k in kw_fin):
            texto = str(df_pre.iloc[-1].get('Operation',''))[:80]
            alertas.append(('error', f'Fin de PRE no coincide con límite DTM ({tipo_pozo}): "{texto}"'))
    if not df_post.empty:
        ...
        if not any(k in op for k in kw_inicio):
            ...
            alertas.append(('error', f'Inicio de POST no coincide con límite DTM ({tipo_pozo}): "{texto}"'))
    return alertas

# ── App ───────────────────────────────────────────────────────────────────────
def main():
    st.title("🛢️ Revisor Unit Rotary — Pulling")
    st.caption("DLS-080 / NOVA-081 · v2.0")

    modelo = cargar_modelo()
    if modelo:
        st.sidebar.success(f"✅ Modelo: {len(modelo['labels'])} ejemplos")
    else:
        st.sidebar.error("Modelo no encontrado — solo reglas")

    # Sidebar
    with st.sidebar:
        st.header("⚙️ Configuración")
        pileta = st.checkbox("Pileta propia (080/081)", value=True)
        tipo_pozo = st.selectbox("Sistema de extracción",
            ["BM","BES","INY","PCP"],
            help="BM=Bombeo Mecánico · BES=Bomba Eléctrica · INY=Inyección · PCP=Cavidad Progresiva")
        st.divider()
        st.header("🔍 Filtros")
        mostrar = st.multiselect("Mostrar", ['DTM','RIG','RIGS','RIGWOW','RIGNC','THR','?'],
                                 default=['DTM','RIG','RIGS','RIGWOW','RIGNC','THR','?'])
        solo_rev = st.checkbox("Solo revisiones")
        busqueda = st.text_input("Buscar en operación", "")
        st.divider()
        st.caption("Revisor Unit Rotary v2.0")
    ...
    try:
        ...
        if 'TIME SUMMARY' not in xls.sheet_names:
            ...
            return
        df_raw = pd.read_excel(xls, sheet_name='TIME SUMMARY', header=0)
    ...
    alertas = validar_limites(df_pre, df_post, tipo_pozo)
    if alertas:
    ...
    c2.metric("✅ Alta", len(df_res[df_res['Confianza']=='alta']))
    c3.metric("⚠️ Media", len(df_res[df_res['Confianza']=='media']))
    c4.metric("🔴 Revisar", len(df_res[df_res['Confianza']=='revision']))
    c5.metric("🔀 Diferencias", int(df_res['_diff'].sum()))
    dists = df_res['App'].value_counts().to_dict()
    c6.metric("DTM/RIG/RIGS", f"{dists.get('DTM',0)}/{dists.get('RIG',0)}/{dists.get('RIGS',0)}")

    st.divider()

    # Filtrar
    mask = df_res['App'].isin(mostrar)
    ...
    # Picklist con emojis para el Super
    TARIFAS_COLOR = [
        '🟢 DTM',
        '🟩 RIG',
        '🔵 RIGS',
    ...
    edited = st.data_editor(
        ...
        column_config={
            ...
            'Operación':   st.column_config.TextColumn('Operation', width=340, disabled=True),
    ...
    # Correcciones y observaciones del editor
    correcciones  = {}
    observaciones = {}
    for vi, (_, row_ed) in enumerate(edited.iterrows()):
    ...
    for pos, row_res in df_res.iterrows():
        if pos not in correcciones:
            correcciones[pos] = row_res['App'] if row_res['App'] != '?' else ''

    n_rev = len(df_res[df_res['Confianza']=='revision'])
    n_sim = int(df_res['_simops'].sum())
    st.caption(
        (f"🔴 {n_rev} a revisar · " if n_rev else '') +
        (f"⚡ {n_sim} SIMOPS · " if n_sim else '') +
        "✏️ Editá la columna Super y escribí observaciones antes de exportar"
    )
